...
# FINRA 1999-01 (published end of Feb) + 1 month delay = effective 1999-03.
...
def run_strategy(df, base_ticker, lev_ticker, cash_ticker, target_lev_weight=0.5, target_base_weight=0.5):
    ...
    for i, row in df.iterrows():
        ...
        if current_month is None or month != current_month:
            ...
            # Determine RED condition with CAPE double filter
            is_red_signal = False
            if signal > 0.45:
                is_red_signal = True
...
print("=== BACKTEST RESULTS (WITH HYSTERESIS + CAPE FILTER) ===")
