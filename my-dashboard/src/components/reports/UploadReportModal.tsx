...
export default function UploadReportModal({ isOpen, onClose, userEmail }: UploadReportModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-background border border-foreground/10 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between p-6 border-b border-foreground/10">
          <h2 className="text-xl font-bold text-foreground">新增投資報告</h2>
          <button onClick={onClose} className="p-2 text-foreground/40 hover:text-foreground hover:bg-foreground/10 rounded-full transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="pt-2">
            <div className="flex space-x-1 p-1 bg-foreground/5 rounded-xl mb-4">
              <button
                type="button"
                disabled
                className="flex-1 flex justify-center items-center gap-2 py-2 text-sm font-medium rounded-lg transition-all text-foreground/40 cursor-not-allowed"
              ...
              <button
                ...
                className={`flex-1 flex justify-center items-center gap-2 py-2 text-sm font-medium rounded-lg transition-all ${fileSource === 'url' ? 'bg-background shadow-sm text-foreground' : 'text-foreground/50 hover:text-foreground/80'}`}
              >
                <LinkIcon size={16} />
                Google Doc 連結
              </button>
