...
export default function ClientFundamentalView({ data }: { data: any }) {
  ...
  if (!data || !data.profile) {
    return <div className="text-white text-center mt-20 text-2xl">找不到該股票資料，請確認代號是否正確。</div>;
  }
        ...
        {[
          ...
          <button
            ...
            className={`px-6 py-3 font-medium transition-all ${
              activeTab === tab.id 
                ? 'text-rose-400 border-b-2 border-rose-400 bg-white/5 rounded-t-lg' 
      ...
      <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-10 shadow-2xl">
        ...
        {activeTab === 'summary' && (
          <div className="space-y-12">
            {/* Chart Section */}
            <div>
              <h3 className="text-2xl font-bold text-white mb-6 border-b border-white/10 pb-4">營收與淨利趨勢 (十億美元)</h3>
            ...
            {/* Profile Info */}
            <div>
              <h3 className="text-2xl font-bold text-white mb-6 border-b border-white/10 pb-4">公司簡介</h3>
              <p className="text-white/70 leading-relaxed text-lg mb-6">{profile.description}</p>
              
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                  <div className="text-white/50 text-sm mb-1">產業 (Industry)</div>
                  <div className="text-white font-medium">{profile.industry}</div>
                </div>
                <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                  <div className="text-white/50 text-sm mb-1">板塊 (Sector)</div>
                  <div className="text-white font-medium">{profile.sector}</div>
                </div>
                <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                  <div className="text-white/50 text-sm mb-1">CEO</div>
                  <div className="text-white font-medium">{profile.ceo}</div>
                </div>
                <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                  <div className="text-white/50 text-sm mb-1">總部</div>
                  <div className="text-white font-medium">{profile.city}, {profile.state}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-12">
            <div className="overflow-x-auto">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-bold text-white">年度損益表 (Annual Income Statement)</h3>
                <span className="text-rose-400/80 text-sm flex items-center"><Activity className="w-4 h-4 mr-1" /> 點擊任意行即可查看歷史趨勢圖</span>
              </div>
              <table className="w-full text-left text-white">
                <thead className="bg-white/10 text-white/80 border-b border-white/20">
                  <tr>
                ...
                <tbody className="divide-y divide-white/10">
                  <tr className="hover:bg-white/10 transition-colors cursor-pointer group" onClick={() => handleIncomeClick('營業收入', 'revenue')}>
                    <td className="p-4 text-white/70 group-hover:text-rose-300 transition-colors">營業收入 (Revenue)</td>
                    {[...income].reverse().map((item: any) => (
                      <td key={item.date} className="p-4">{(item.revenue / 1000000).toLocaleString(undefined, {maximumFractionDigits: 0})}</td>
                    ))}
                  </tr>
                  <tr className="hover:bg-white/10 transition-colors cursor-pointer group" onClick={() => handleIncomeClick('營業成本', 'costOfRevenue')}>
                    <td className="p-4 text-white/70 group-hover:text-rose-300 transition-colors">營業成本 (Cost of Revenue)</td>
                    {[...income].reverse().map((item: any) => (
                      <td key={item.date} className="p-4">{(item.costOfRevenue / 1000000).toLocaleString(undefined, {maximumFractionDigits: 0})}</td>
                    ))}
                  </tr>
                  <tr className="hover:bg-white/10 transition-colors font-medium text-indigo-300 cursor-pointer group" onClick={() => handleIncomeClick('毛利', 'grossProfit')}>
                  ...
                  <tr className="hover:bg-white/10 transition-colors cursor-pointer group" onClick={() => handleIncomeClick('營業費用', 'operatingExpenses')}>
                    <td className="p-4 text-white/70 group-hover:text-rose-300 transition-colors">營業費用 (Operating Expenses)</td>
                    {[...income].reverse().map((item: any) => (
                      <td key={item.date} className="p-4">{(item.operatingExpenses / 1000000).toLocaleString(undefined, {maximumFractionDigits: 0})}</td>
                    ))}
                  </tr>
                  <tr className="hover:bg-white/10 transition-colors font-medium text-indigo-300 cursor-pointer group" onClick={() => handleIncomeClick('營業利益', 'operatingIncome')}>
                  ...
                  <tr className="hover:bg-white/10 transition-colors font-bold text-rose-400 cursor-pointer group" onClick={() => handleIncomeClick('淨利', 'netIncome')}>
                  ...
                  <tr className="hover:bg-white/10 transition-colors cursor-pointer group" onClick={() => handleIncomeClick('每股盈餘 (EPS)', 'eps', 1)}>
                    <td className="p-4 text-white/70 group-hover:text-rose-300 transition-colors">每股盈餘 (EPS)</td>
                    {[...income].reverse().map((item: any) => (
            ...
            {selectedChart && (
              <div className="bg-[#111] border border-rose-500/30 rounded-2xl p-6 shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {activeTab === 'ratios' && (
          <div className="space-y-12">
            <div className="overflow-x-auto">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-bold text-white">年度關鍵指標 (Key Metrics)</h3>
                <span className="text-rose-400/80 text-sm flex items-center"><Activity className="w-4 h-4 mr-1" /> 點擊任意行即可查看歷史趨勢圖</span>
              </div>
              <table className="w-full text-left text-white">
                <thead className="bg-white/10 text-white/80 border-b border-white/20">
                ...
                <tbody className="divide-y divide-white/10">
                  <tr className="hover:bg-white/10 transition-colors cursor-pointer group" onClick={() => handleMetricClick('本益比估算', 'earningsYield', false, true)}>
                    <td className="p-4 text-white/70 group-hover:text-rose-300 transition-colors">本益比估算 (Implied P/E)</td>
                    {[...metrics].reverse().map((item: any) => (
                      <td key={item.date || item.fiscalYear} className="p-4">{item.earningsYield ? (1 / item.earningsYield).toFixed(2) : '-'}</td>
                    ))}
                  </tr>
                  <tr className="hover:bg-white/10 transition-colors cursor-pointer group" onClick={() => handleMetricClick('企業價值倍數', 'evToEBITDA', false, false)}>
                    <td className="p-4 text-white/70 group-hover:text-rose-300 transition-colors">企業價值倍數 (EV/EBITDA)</td>
                    {[...metrics].reverse().map((item: any) => (
                      <td key={item.date || item.fiscalYear} className="p-4">{item.evToEBITDA?.toFixed(2) || '-'}</td>
                    ))}
                  </tr>
                  <tr className="hover:bg-white/10 transition-colors cursor-pointer group" onClick={() => handleMetricClick('股東權益報酬率', 'returnOnEquity', true, false)}>
                    <td className="p-4 text-white/70 group-hover:text-rose-300 transition-colors">股東權益報酬率 (ROE)</td>
                    {[...metrics].reverse().map((item: any) => (
                      <td key={item.date || item.fiscalYear} className="p-4">{((item.returnOnEquity || 0) * 100).toFixed(2)}%</td>
                    ))}
                  </tr>
                  <tr className="hover:bg-white/10 transition-colors cursor-pointer group" onClick={() => handleMetricClick('資產報酬率', 'returnOnAssets', true, false)}>
                    <td className="p-4 text-white/70 group-hover:text-rose-300 transition-colors">資產報酬率 (ROA)</td>
                    {[...metrics].reverse().map((item: any) => (
                      <td key={item.date || item.fiscalYear} className="p-4">{((item.returnOnAssets || 0) * 100).toFixed(2)}%</td>
                    ))}
                  </tr>
                  <tr className="hover:bg-white/10 transition-colors cursor-pointer group" onClick={() => handleMetricClick('自由現金流殖利率', 'freeCashFlowYield', true, false)}>
                    <td className="p-4 text-white/70 group-hover:text-rose-300 transition-colors">自由現金流殖利率 (FCF Yield)</td>
                    {[...metrics].reverse().map((item: any) => (
                      <td key={item.date || item.fiscalYear} className="p-4">{((item.freeCashFlowYield || 0) * 100).toFixed(2)}%</td>
                    ))}
                  </tr>
                  <tr className="hover:bg-white/10 transition-colors cursor-pointer group" onClick={() => handleMetricClick('流動比率', 'currentRatio', false, false)}>
                    <td className="p-4 text-white/70 group-hover:text-rose-300 transition-colors">流動比率 (Current Ratio)</td>
                    {[...metrics].reverse().map((item: any) => (
